...
> 🔶 **2026-07-07 (проход 4 — крупная ребалансировка данных). Числа ниже частично устарели.**
> Оракул теперь — **`calibrate.py`** (coupled: реальная стоимость обороны ↔ добыча, mixed-defense
> grid-модель), а не `difficulty/econ_timing/pacing_v2`. Что изменилось в `data/*.json` и коде:
