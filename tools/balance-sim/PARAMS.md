...
> 🔶 **2026-07-08. Числа ниже во многом УСТАРЕЛИ — этот файл теперь исторический инвентарь.**
> Оракул петли — **`sim.py`** (не `calibrate.py`, и тем более не удалённые
> `difficulty/econ_timing/pacing_v2`). Единственный источник истины по числам — **`data/*.json`**;
> полная карта переменных и связей — **`plans/balance-variables-map.md`**. Схема из 4 симов и
> запуск — в `README.md`. Что уже НЕ так, как в тексте ниже: цена зданий перешла на **level-sum**
> кривую (Σ2^(ур−1), фактор 1.11/wall 1.06); добавлен **ability-accumulation sink** (1.6); награды
> за волны теперь **моделируются в `sim.py`** (компаунд perm-карт ≈ ×3 к добыче); капстоуны в
> `sim.py` смоделированы (строка «capstones не смоделированы» ниже — историческая).
>
> ⟨исторический контекст прохода 4⟩ Что изменилось в `data/*.json` и коде:
> здания получили тиры **L4/L5** (актив переехал на L5); слоты шахт урезаны до дефицитных (3, у
> wood/ore 4-й на в14); формула ремонта исправлена на долевую (`repairCostPerHpFractionOfBuyCost`
> 0.02→**0.5**); турель стала ore-heavy (ore70/iron22); добыча пересобрана (5/12/28/64/145/…);
> **скейл врагов усилен** до `hp + 5(w-1) + 0.35(w-1)²`, atk `+(w-1)//2`. Полная карта всех
> переменных и связей: **`plans/balance-variables-map.md`**.
>
> **Незакрытое (для нового витка сима):** поздние стойла в `calibrate.py` — артефакт жадной
> аллокации бота (бросает wood ради ore/iron) + замороженный ростер (экспонента). Combat-кривая
> уже здоровая (полоса ретраев 8/24, боссы-пики). Сим требует переосмысления (см. §7 карты).
...
### Статус исправлений (третий проход, done)

| # | Пункт | Статус |
|---|---|---|
| 1 | enemy attack scaling `//2` → `//3` | ✅ battle_sim.py |
| 2 | boss mechanics (aura/summon/breach) в battle_sim | ✅ tick_boss_mechanic + inline breach mult |
| 3 | waveDemand.slotProductionMultiplier | ✅ pacing_v2 + econ_timing применяют × 1.25 на подсвеченной шахте |
| 4 | attrition damageFloor в econ | ✅ econ_timing: `DEFEAT_WAVES` override + accumulated repair tax |
| 5 | repair как сток | ✅ вшит в defeat-ветку econ_timing (совместно с #4) |
| 6 | pacing_v2 merge_cap = 5 | ✅ убран динамический рост |
| 7 | early-start бонус | ✅ pacing_v2 + econ_timing применяют линейный decay |
| 8 | reward draft picks | ✅ econ_timing: rotation permanent gold/res + temporary prod |
| 9 | Golden trait gold trickle | ✅ pacing_v2 + econ_timing (аппроксимация «1/3 workers golden-dominant»); Rush/Shift/capstones — отложено |
| 10 | building actives + shields | ⏸ отложено до late-game калибровки (TODO в коде + task #15) |
| 11 | start-of-wave attrition в battle_sim | ✅ Building.damageFloor поле; hp = maxHp × max(0, post − floor) на старте |
| 12 | battle_sim читает fortress-buildings.json | ✅ `_load_buildings()` с fallback |
