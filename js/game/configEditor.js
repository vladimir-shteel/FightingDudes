...
const HINTS = {
  ...
  fortressBuildings: "Per-building footprint, buy cost, and per-level stats (hp/damage/cooldown/active). One entry per building type.",
  ...
  fortress: "Field setup: starting obstacle coverage, the cost curve for clearing them, and the fallback repair rate for buildings with no buyCost (the HQ).",
  "fortress.obstaclePercentMin": "Lower bound (fraction of the tiles not already taken by the HQ/starting buildings) for how much of the open field starts covered in scenery obstacles (trees). Randomized per run between min and max, rounded up.",
  "fortress.obstaclePercentMax": "Upper bound (fraction of the open, non-reserved tiles) for starting obstacle coverage.",
